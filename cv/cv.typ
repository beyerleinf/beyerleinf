...
#let addresstext(info, uservars) = {
    if uservars.showAddress {
        block(width: 100%)[
            #info.personal.location.region, #info.personal.location.country
            #v(-4pt)
        ]
    } else {none}
}
...
#let cveducation(info, isbreakable: true) = {
    if info.education != none {block[
        ...
        #for edu in info.education {
            ...
            let edu-items = ""
            if "honors" in edu {edu-items = edu-items + "- *Honors*: " + edu.honors.join(", ") + "\n"}
            if "courses" in edu {edu-items = edu-items + "- *Courses*: " + edu.courses.join(", ") + "\n"}
